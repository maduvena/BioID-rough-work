...
public class BioIDService {
	...
	public List<BioIDCredential> getBioIDDevices(String userId) {
		...
		try {
			...
			devices = mapper.readValue(json, new TypeReference<List<BioIDCredential>>() {
			});

			logger.trace("getBioIDDevices. User '{}' has {}", userId,
					devices.stream().map(BioIDCredential::getBcid).collect(Collectors.toList()));
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
		}
		return devices;
	...
	public boolean writeToPersistence(String bcid, String task, String trait, String userId)
		...
		if (BioIDService.getInstance().TASK_ENROLL.equals(task)) {
			BioIDCredential cred = new BioIDCredential(bcid, trait, System.currentTimeMillis());
